...
class canvas
{
	...
	if ( is_file( $this->origem ) )
	{
	    ...
	    if ( !$this->eImagem() )
	    {
		trigger_error( 'Error: Archivo '.$this->origem.' no es imagen!'.@header( 'Location: home' ).'' , E_USER_ERROR );
	    }
	    else
	    {
		// pega dimensões
	...
	{
	    trigger_error( 'Error: Archivo no encontrado!'.@header( 'Location: home' ).'', E_USER_ERROR );
	}
